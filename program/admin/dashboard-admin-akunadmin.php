<?php
    ...
    $admin = query("SELECT * FROM akun_admin");

    if (isset($_POST["submit"])) {
        
        $id = $_POST["id"];

        mysqli_query($koneksi, "DELETE FROM akun_admin WHERE id_adm = $id");
    }
?>
<!doctype html>
<html lang="en">
        ...
        <div class="container-sm">
            <div class="row">
                ...
                <div class="col" id="kol-dashboard" style="margin-right:-100px">
                    <div class="row" id="header-dashboard" style="margin-right:-300px">
                        <div class="col">
                            <span> 
                    ...
                    <a class="btn btn-primary" href="dashboard-admin.php" role="button">Kembali</a>
                    <a class="btn btn-primary" href="dashboard-admin-tambah-admin.php" role="button">Tambah Admin</a>
                    <a class="btn btn-success" href="dashboard-admin-akunuser.php" role="button">Lihat Data Pengguna</a>

                    <div class="card mt-5">
                        <h5 class="card-header"> Akun Admin</h5>
                                ...
                                <tbody>
                                    <?php foreach ($admin as $row) : ?>
                                        <tr>
                                            <td><?=$row["id_adm"];?></td>
                                            <td><?=$row["nama_adm"];?></td>
                                            <td><img src="../../img/upload/<?=$row['foto_adm'];?>" alt="" width="40"></td>
                                            <td><?=$row["tempat_lhr_adm"];?></td>
                                            <td><?=$row["tgl_lhr_adm"];?></td>
                                            <td><?=$row["alamat_adm"];?></td>
                                            <td><?=$row["nmpengguna_adm"];?></td>
                                            <td><?=$row["email_adm"];?></td>
                                            <td><?=$row["telepon_adm"];?></td>
                                            <td>
                                                <form action="" method="post">
                                                    <input type="hidden" name="id" value="<?=$row["id_adm"];?>" id="">
